...
class compare():
    ...
    def networkData(self, i, networkFitness, fitnessParent, fitnessChild, data, child):
        if (networkFitness[i] < fitnessParent[i]) or (networkFitness[i] < fitnessChild[i]):
            if fitnessParent[i] > fitnessChild[i]:
                networkFitness[i] = fitnessParent[i]
            else:
    ...
    def BestAndPoorest(self, networkFitness):
